...
//topic 头文件
#include <iostream>
#include <velocity_marker_land/command.h>
...
using namespace std;
//using namespace velocity_marker_land;
...
//-----------------------------------------视觉相关----------------------------------------------------
float flag_vision = 1;                                          //视觉FLAG 是否识别到目标 1代表能识别到目标，0代表不能
...
//---------------------------------------Output---------------------------------------------
velocity_marker_land::command Command_now;                               //发送给position_control.cpp的命令
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>函数声明<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
{
    flag_vision = int(msg->orientation.w != 0);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber relative_pos_sub = nh.subscribe<geometry_msgs::Pose>("/apriltags/relative_position", 10, relative_position_cb);

    //【订阅】降落板与无人机的相对偏航角 单位：弧度
    // 利用orientation.w 传递
    ros::Subscriber relative_yaw_sub = nh.subscribe<geometry_msgs::Pose>("/apriltags/relative_yaw", 10, relative_yaw_cb);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>参数读取<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    //允许降落最大距离阈值
    nh.param<float>("Thres_distance_land", Thres_distance_land, 3.0);
    ...
    while (ros::ok())
    {
        ...
        switch (Num_StateMachine)
        {
            ...
            case 4:
                ...
                {
                    cout<< "Flag_z_below_30cm: " <<relative_position.z<< " flase" <<endl;
                    Flag_z_below_30cm = 0;
                }
